                    ...
                    if (!$userthreadSql || mysqli_num_rows($userthreadSql) == 0) {
                        ...
                        while ($ticthreadRes = $userthreadSql->fetch_assoc()) {
                            ...
                            if($rowcount > 0){
                                ...
                                $tmp["message"] = $ticthreadRes["message"];
				$tmp["rate"] = $ticketRes['rate'];
                	        $tmp["callback"] = $ticketRes['callback'];
                		$tmp["stu_remarks"] = $ticketRes['stu_remarks'];
		                $tmp["stu_remarks_date"] = $ticketRes['stu_remarks_date'];
...
function createtickets($params)
 ...
 function getthreadDetails($params)
    ...
    function getstudentrating($params)
    {
        global $conn;
        ...
        $userSql="SELECT * FROM `uv_ticket` where id=$tic_id";
        ...
        if (!$userSql || mysqli_num_rows($userSql) == 0) {
            ...
            while ($data = $userSql->fetch_assoc()) {
                ...
                $tmp["stu_remarks"] = $data['stu_remarks'];
                $tmp["stu_remarks_date"] = $data['stu_remarks_date'];
    ...
    function addstudentrating($params)
    {
        global $conn;
        ...
        $stu_remarks = $params['stu_remarks'];
        ...
        if (!$userSql || mysqli_num_rows($userSql) == 0) {
            ...
            $userSql="UPDATE `uv_ticket` SET `rate` = '$rate', `callback` = '$callback', `stu_remarks` = '$stu_remarks', `stu_remarks_date` = '$date'  WHERE `uv_ticket`.`id` = '$tic_id'";
